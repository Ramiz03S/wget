...
void static init_response_components_t(response_components_t * recv){
    recv->content_length = -1;
    recv->status = -1;
    recv->chunked_encoding = -1;
}

void static form_get_req(char * host, char * path, char * get_req_buffer){
    if(path == NULL){
        path = "/";
    }
...
void static parse_status_headers(FILE * fptr, size_t data_idx, response_components_t * response_components){
